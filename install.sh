...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/AikoCute/Aiko-XrayR/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/AikoCute/Aiko-XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/AikoCute/Aiko-XrayR/releases/download/${last_version}/XrayR-linux-${arch}.zip"
    ...
    file="https://raw.githubusercontent.com/AikoCute/Aiko-XrayR/data/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "Cài đặt mới, vui lòng tham khảo hướng dẫn trước：https://github.com/AikoCute/Aiko-XrayR，Định cấu hình nội dung cần thiết"
    else
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR Có thể không khởi động được, vui lòng sử dụng sau XrayR log Kiểm tra thông tin nhật ký, nếu không khởi động được, định dạng cấu hình có thể đã bị thay đổi, vui lòng vào wiki để kiểm tra：https://github.com/AikoCute/Aiko-XrayR/wiki${plain}"
        fi
    fi
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/AikoCute/Aiko-XrayR/data/XrayR.sh
