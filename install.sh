...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/AikoCute/AikoXrayR-install/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/AikoCute/AikoXrayR-install/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/AikoCute/AikoXrayR-install/releases/download/${last_version}/XrayR-linux-${arch}.zip"
    ...
    file="https://raw.githubusercontent.com/AikoCute/AikoXrayR-install/master/XrayR.service"
